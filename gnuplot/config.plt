...
set yrange [-0.2:1.2];
set autoscale
plot '< tail -n 555 log.txt' every ::1 using 1:2 with   lines linestyle 3 notitle, \
     '< tail -n 555 log.txt' every ::1 using 1:3 with   lines linestyle 4 notitle,

set ylabel "speed"
set title "0-1 deg/seg"
set yrange [-0.2:1.2];
set autoscale
plot '< tail -n 555 log.txt' every ::1 using 1:4 with   lines linestyle 3 notitle, \
     '< tail -n 555 log.txt' every ::1 using 1:5 with   lines linestyle 4 notitle,

set title "torque"
set ylabel "Nm - rev/seg"
set yrange [0:0.15];
set autoscale
plot '< tail -n 555 log.txt' every ::1 using 1:6 with   lines  linestyle 1 notitle, \
  '< tail -n 555 log.txt' every ::1 using 1:7 with   lines  linestyle 2 notitle

...
float32_t lsw1Speed = 0.01;                // initial force rotation speed in search
                                           // of QEP index pulse
...
void main(void)
{
    ...
    // Setting up link from EPWM to ADC - EPWM1 - Inverter currents at sampling frequency (@ PRD or @ (PRD&ZRO) )
    EPWM_setADCTriggerSource        ( EPWM1_BASE, EPWM_SOC_A, EPWM_SOC_TBCTR_PERIOD ); // Select SOC from counter at ctr = prd
    ...
    EPWM_setInterruptSource             ( EPWM11_BASE, EPWM_INT_TBCTR_U_CMPA ); // Enable EPWM11 INT to reset SDFM in sync with control PWMs Select INT @ ctr = CMPA up
    EPWM_enableInterrupt                ( EPWM11_BASE                        ); // Enable INT
    EPWM_setInterruptEventCount         ( EPWM11_BASE, 1                     ); // Generate INT on every event
    EPWM_clearEventTriggerInterruptFlag ( EPWM11_BASE                        );

    // Enable EPWM1 INT to generate MotorControlISR
    EPWM_setInterruptSource             ( EPWM1_BASE, EPWM_INT_TBCTR_PERIOD ); // Select INT @ ctr = 0
    EPWM_enableInterrupt                ( EPWM1_BASE                        ); // Enable Interrupt Generation from the PWM module
    ...
    ADC_setPPBReferenceOffset(ADCA_BASE, ADC_PPB_NUMBER1, (uint16_t)(offset_lemV*4096.0)); // setting LEM Iv offset
    ADC_setPPBReferenceOffset(ADCB_BASE, ADC_PPB_NUMBER1, (uint16_t)(offset_lemW*4096.0)); // setting LEM Iw offset
       ...
       if(lsw == QEP_ALIGNMENT) {
          ...
          if(pi_id.ref >= IdRef) {
             if(++alignCntr >= alignCnt) {
                alignCntr = 0;
                IdRef     = IdRef_run;
                lsw      = QEP_WAIT_FOR_INDEX; // for QEP, spin the motor to find the index pulse
             }
          }
       }
       else
